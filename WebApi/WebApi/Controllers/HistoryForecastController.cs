...
{
    public class HistoryForecastController : ApiController
    {
        [HttpGet]
        ...
        public IHttpActionResult Get(String staName)
        {
            ...
            try
            {
                ...
                List<Object> historyForecastList = new List<object>();
                if (reader.HasRows)//如果有数据
                {
                    while (reader.Read())//初始索引是-1，执行读取下一行数据，返回值是bool
                    {
                        historyForecastList.Add(new
                        {
                            staName =(string)reader["f_sta"],
                            date = reader["f_date"],
                            forecastIn = reader["f_daily_in"],
                            forecastOut = reader["f_daily_out"],
                            forecastError = reader["f_daily_error"],
                        }) ;
                    }
                }
                return Json(historyForecastList);
            }
        ...
        public IHttpActionResult Get()
        {
            ...
            {
                conn.Open();//建立连接
                string sql = "select * from forecast_daily";
                MySqlCommand cmd = new MySqlCommand(sql, conn);
                MySqlDataReader reader = cmd.ExecuteReader();//执行ExecuteReader()返回一个MySqlDataReader对象
                List<Object> historyForecastList = new List<object>();
                if (reader.HasRows)//如果有数据
